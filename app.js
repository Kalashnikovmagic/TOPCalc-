...
document.addEventListener("pointerup", e=>{
  ...
  if(k==="%"){
    if(mode==="normal"){
      mode="Data";
      current="0"; X=null; fullY=""; Y=""; waitingForY=false; Z=0;
      update();
      updateClearButton();
    }
    return;
  ...
  if(!isNaN(k)){
    if(waiting){ current=k; waiting=false; } else { current=current==="0"?k:current+k; }
    update(); return;
  }
...
document.addEventListener("touchstart", e => { 
  if(e.touches.length === 3){ active=true; startY=[...e.touches].reduce((a,t)=>a+t.clientY,0)/3; } 
}, {passive:true});
...
// ===== NO ZOOM / NO SCROLL / NO TEXT SELECT =====
